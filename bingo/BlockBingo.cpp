...
int BlockBingo::selectCarry()
{
    ...
    */
    tmpNodeid = mRunner->getStart()->getNodeid();
    ...
    for (node_idx=0;node[node_idx]!=-1;node_idx++) {
     ...
     //   mDistance->debugPrint2();
       ...
       // dummy->setDir(runner_dir);
        ...
        double mincost2=18; // ブロック色不明又はブロック退避の場合のペナルティ（要調整）
        int minidx=-1;
...
//
      //   DIR runner_dir = mDistance->getGoalDirection(st_node); // 開始位置の方向を予測
       ...
       // mDistance->debugPrint2();
        ...
        //mDistance->debugPrint();
        //sprintf(buf,"carry:%f %d",mincost2,list[minidx]);
        //msg_f(buf,12);
...
int BlockBingo::toExit()
{
    ...
    tmpRunner = mRunner->makeClone();
    */ 
    tmpNodeid = dummy->getStart()->getNodeid();
    tmpDir = dummy->getDir();
